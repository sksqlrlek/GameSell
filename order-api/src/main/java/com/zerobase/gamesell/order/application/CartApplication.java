...
public class CartApplication {
  ...
  public Cart addCart(Long userId, AddGameCartForm form) {
    ...
    Game game = gameRepository.findBySellerIdAndId(
        form.getSellerId(), form.getId())
  ...
  public Cart removeGameCart(Long userId, Long gameId) {
    ...
    boolean removed = cart.getGames().removeIf(g -> g.getId().equals(gameId));
    if (removed) {
  ...
  public CartResponse getCart(Long userId) {
    Cart cart = cartService.getCart(userId);
    CartResponse response = refreshCart(cart);
    cartService.putCart(userId, response.getCart());
    return response;
  }


  private CartResponse refreshCart(Cart cart) {
    ...
    UserDto userDto = userClient.getUser(cart.getUserId());
    int userAge = Period.between(userDto.getBirthDate(), LocalDate.now()).getYears();
  ...
  public void clearCart(Long userId) {
    cartService.putCart(userId, null);
  }
}
